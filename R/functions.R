...
get_tt_image <- function(year, week) {
  ...
  destination <- paste0("gallery/img/", year, "-", week, "-", 
                        trimws(basename(origin)))
  
  if (!file.exists(destination)) {
    if (!file.exists("gallery/img")) {
      dir.create("gallery/img")
    }
    download.file(origin, destination, method = "curl")
  }
  
  thumb_destination <- paste0("gallery/img/thumb-", year, "-", week, "-", 
                              trimws(basename(origin)))
...
resize_image <- function(image) {
  ...
  imFile_resized <- magick::image_resize(imFile, "x200")
